...
class MDTrieIndexTests: XCTestCase {
    ...
    override func setUp() {
        ...
        trainingList = normalise(stringList: string.components(separatedBy: CharacterSet(charactersIn: ",.")))
    }
    ...
    func testTraining() {
        ...
        self.measure {
            ...
            trieIndex.train(trainList: self.trainingList)
        }
    }
    ...
    func testFindAllStrings(){
        ...
        let normalizedTrainingList = normalise(stringList: trainingList).sorted()
        let sortedAllStrings = allStrings.sorted()
        
        for (str1,str2) in zip(normalizedTrainingList, sortedAllStrings){
    ...
    func normalise(stringList : [String])->[String]{
        ...
        for str in stringList{
            if str != ""{
                nonEmptystringList.append(str)
            }
        }
        
        let stringsSet = Set(nonEmptystringList)
        return Array(stringsSet)
    }
    
    func testFindAllStringsWithPrefix(){
        ...
        for (str1,str2) in zip(trainingList.sorted(), allStrings.sorted()){
            XCTAssertTrue(str1 == str2)
        }
